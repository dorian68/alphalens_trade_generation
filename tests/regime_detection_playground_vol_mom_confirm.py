...
def _print_summary(
    ...
    print("\nRegime detection summary (volume/momentum confirmation)")
...
def main() -> None:
    ...
    if "volume" not in df.columns:
        print("Volume not available in data; vol/mom confirmation will be limited.")

    detector = DeterministicRegimeDetector(
        RegimeConfig(
            regime_mode="heuristic",
            enable_vol_mom_confirm=True,
            volume_window=50,
    ...
    _plot_regime_series(df, series, f"{symbol} ({timeframe}) regime detection - vol/mom confirm")
