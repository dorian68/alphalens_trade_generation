...
def _print_summary(
    ...
    print("\nRegime detection summary (heuristic)")
...
def main() -> None:
    ...
    detector = DeterministicRegimeDetector(RegimeConfig(regime_mode="heuristic"))
    result = detector.detect(df)
    series = detector.detect_series(df)

    _plot_regime_series(df, series, f"{symbol} ({timeframe}) regime detection - heuristic")
