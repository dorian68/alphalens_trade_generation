...
def _print_summary(
    ...
    print("\nRegime detection summary (walk-forward calibration)")
...
def main() -> None:
    ...
    detector = DeterministicRegimeDetector(
        RegimeConfig(
            ...
            enable_walk_forward_calib=True,
            calib_window=200,
            calib_min_history=50,
            calib_trend_quantile=0.7,
            calib_range_quantile=0.3,
            calib_chop_quantile=0.7,
            calib_vol_expansion_quantile=0.7,
        )
    )
    result = detector.detect(df)
    series = detector.detect_series(df)

    _plot_regime_series(df, series, f"{symbol} ({timeframe}) regime detection - walk-forward calib")
